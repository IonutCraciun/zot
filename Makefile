...
.PHONY: test
test:
	$(shell mkdir -p test/data;  cd test/data; ../scripts/gen_certs.sh)
	sudo -E skopeo --insecure-policy copy -q docker://centos:latest oci:${TOP_LEVEL}/test/data/zot-test:0.0.1
	sudo -E skopeo --insecure-policy copy -q docker://centos:8 oci:${TOP_LEVEL}/test/data/zot-cve-test:0.0.1
